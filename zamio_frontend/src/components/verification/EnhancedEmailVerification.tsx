...
export function EnhancedEmailVerification({
  ...
  const handleCodeComplete = async (code: string) => {
    ...
    try {
      const response = await api.post('api/accounts/verify-email-code/', {
        email,
        code
      });
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const errorMessage = err?.response?.data?.message || 'Failed to resend verification. Please try again.';
      setError(errorMessage);
    } finally {
      setIsVerifying(false);
    }
