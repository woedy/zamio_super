...
export function EnhancedPasswordReset({
  ...
  const handleSendReset = async () => {
    ...
    try {
      const response = await fetch(baseUrl + 'api/accounts/request-password-reset/', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        const errorMessage = data?.message || 'Failed to send password reset. Please try again.';
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(baseUrl + 'api/accounts/confirm-password-otp/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
          otp_code: '123456', // This would be the actual code from CodeInput
  ...
  const handleResend = async () => {
    ...
    try {
      const response = await fetch(baseUrl + 'api/accounts/resend-password-otp/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
      ...
      if (response.ok) {
        ...
        const errorMessage = data?.message || 'Failed to resend password reset. Please try again.';
        setError(errorMessage);
      }
    } catch (err) {
      setError('Failed to resend password reset. Please try again.');
    } finally {
      setIsVerifying(false);
    }
  };
