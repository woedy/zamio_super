...
export default function SignIn() {
  ...
  interface AdminLoginData extends Record<string, unknown> {
    next_step?: string;
    onboarding_step?: string;
  }

  type AdminLoginResponse = LegacyLoginResponse & {
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-slate-950 text-white overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-slate-400">Sign in to access the administrative dashboard</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-white/10 bg-slate-900/60 p-8 shadow-2xl backdrop-blur">
          ...
          <p className="text-center text-sm text-slate-400">
            Need admin access?{' '}
            <Link to="/contact" className="font-medium text-indigo-400 hover:text-indigo-300">
              Contact support
            </Link>
          </p>
        </form>
        ...
        <div className="mt-6 rounded-xl border border-indigo-500/20 bg-indigo-500/10 p-4">
          <div className="flex items-center justify-center space-x-2 mb-2">
            ...
            <span className="text-sm font-medium text-indigo-300">Admin Access</span>
          </div>
