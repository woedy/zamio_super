    ...
    try {
      ...
      if (data?.errors) {
        const message = Object.values(data.errors)
          ...
          .join('\n');
        setPasswordError(message || 'Failed to reset password. Please try again.');
      } else {
    ...
    try {
      ...
      if (data?.errors) {
        const message = Object.values(data.errors)
          ...
          .filter((item): item is string => typeof item === 'string')
          .join('\n');
        setError(message || 'Failed to resend password reset. Please try again.');
      } else {
        setError(data?.message || 'Failed to resend password reset. Please try again.');
      }
    } finally {
      setIsVerifying(false);
    }
